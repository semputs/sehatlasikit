...
for row in reader:
    ...
    if date_val and date_val.lower() != 'date':
        ...
        # Extract latest non-empty weight (handles numbers like "82.2" or "83.8 kg")
        if weight_val not in ['--', '', '-', 'None', 'null', 'nan']:
            clean_wt_match = re.search(r'\d+(?:\.\d+)?', weight_val)
            if clean_wt_match:
                wt_num = float(clean_wt_match.group(0))
                if wt_num > 50.0:  # Safeguard for valid human weight
...
# Update window.dashboardData JSON
...
# Safe RegEx replacer that handles classes and attributes inside HTML tags safely
def replace_element_inner_html(html, element_id, new_content):
    pattern = rf'(<[^>]*\bid=["\']{element_id}["\'][^>]*>)(.*?)(</[^>]+>)'
    if re.search(pattern, html, flags=re.DOTALL):
        return re.sub(pattern, rf'\g<1>{new_content}\g<3>', html, flags=re.DOTALL)
    print(f"Warning: Element ID '{element_id}' not found in HTML.")
    return html

# Replace stat card values and dates in index.html
updated_html = replace_element_inner_html(updated_html, "kpi-weight", latest_weight)
updated_html = replace_element_inner_html(updated_html, "kpi-speed", peak_speed_str)
updated_html = replace_element_inner_html(updated_html, "kpi-distance", peak_dist_str)

updated_html = replace_element_inner_html(updated_html, "kpi-weight-date", f"as of {latest_weight_date}" if latest_weight_date else "--")
updated_html = replace_element_inner_html(updated_html, "kpi-speed-date", f"hit on {peak_speed_date}" if peak_speed_date else "--")
updated_html = replace_element_inner_html(updated_html, "kpi-distance-date", f"hit on {peak_distance_date}" if peak_distance_date else "--")

# Write updated index.html
with open('index.html', 'w', encoding='utf-8') as f:
    f.write(updated_html)

print(f"KPI Updated: Wt={latest_weight} ({latest_weight_date}), Spd={peak_speed_str} ({peak_speed_date}), Dist={peak_dist_str} ({peak_distance_date})")
